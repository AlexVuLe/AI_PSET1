...
function pickedTags(){
	...
	// special symbols treatment
	if (tagname.indexOf('%') > -1 ) {
            tagname = tagname.replace(/\%/g, "%25");
        }
        if (tagname.indexOf('+') > -1) {
            tagname = tagname.replace(/\+/g, "%2B");
        }
	if (tagname.indexOf('#') > -1) {
	...
	if (tagname == '') {
                tagname = $.trim($(obj).attr('rel'));
		reason =  $.trim($(obj).attr('value'));
		if (reason == 'On') {
			reason = 'good';
		} else {
			reason = 'bad';
	...
	var special_target = specialTags;
	...
	var special_re = RegExp('^' + special_prefix);
        interestingTags = {};
        ignoredTags = {};
	specialTags = {};
        $('.deletable-tag').each(
		...
		else if (special_re.test(item_rel)){
			if (item_value == 'On') {
                        	reason = 'good';
                	} else {
                        	reason = 'bad';
    ...
    return {
	    ...
	    $(".subscriptionBox .deletable-tag").click(function(){handlePickedTag(this,'good');});
